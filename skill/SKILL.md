...
> 진단 결과를 "그러니 Claude를 덜 쓰세요"로 번역하는 순간 이 스킬은 해로워진다. 목표는 **같은 결과를 더 싸게 얻어, 아낀 예산을 실제 작업에 쓰는 것**이다. 토큰을 아껴서 할 일을 못 하는 것은 낭비보다 비싼 실수다. (이 긴장에 대한 근거와 반론은 `references/measurement_and_hooks.md` §5 참고.)
...
0. **[필수·먼저 실행] 진단 범위를 정하고 habit_scan.py 실행**
   ...
   > **서브에이전트 트랜스크립트는 기본 제외된다.** 이 스킬은 *운전자의 습관*을 채점하지, 에이전트가 자기 컨텍스트에서 한 일을 사용자에게 청구하지 않는다. 굳이 포함해 보려면 `--include-subagents`.
...
4. **라우팅·지불 점검** *(CLI가 잡거나, 아예 못 잡는 축)*
   - 기간 내 모델이 100% 최상위 티어인가? → H4-04 → 작업 성격별로 티어를 정해 **새 세션에서** 시작. 린트·포맷·리네임은 모델 없이 도구로
   - 세션 중 모델·MCP·플러그인을 토글했나? → H4-03 → 구성 변경은 세션 시작 전에
   - **구독을 몇 개 결제 중인가? 그중 최근 30일 실사용 0인 것은?** → H9-01 → 해지. **CLI가 절대 못 잡는 축이니 진단 때 반드시 말로 물어라** — 토큰을 아무리 줄여도 미사용 구독료는 그대로 나간다
...
> **점수에 들어가는 것과 안 들어가는 것.** H2-02·H8-02·H4-03·H1-03은 로그의 **실측 토큰 수**로 낭비를 계산한다("실측 낭비"로 표기). H2-01·H5-04·H8-01은 시나리오 상수 × 횟수의 **추정치**다("추정 낭비"). H1-01·H2-04·H4-04·H8-03은 **빈도 신호**일 뿐 Waste Score에 합산하지 않는다 — 장시간 세션·웹 툴·서브에이전트·상위 티어는 그 자체로 낭비가 아니기 때문이다. 사용자에게 점수를 설명할 때 이 구분을 뭉개지 말 것.
...
| 7 | **재읽기 전 컨텍스트 확인** — 이미 이 세션에서 읽은 파일을 다시 Read 요청받으면 컨텍스트 내 참조를 먼저 시도한다. 단 `offset`/`limit`로 나눠 읽는 것은 정상이므로 경고하지 않는다 | 파일 Read 요청 시 |
...
> **로직 SSOT는 레포 루트 `tokenhabit/` 패키지 하나다.** `scripts/habit_scan.py`는 얇은 진입점이고, `_vendor/`는 pip 설치 없이도 스킬이 돌게 하려고 둔 복사본이다. 탐지 규칙을 고칠 일이 생기면 `tokenhabit/`을 고치고 `python3 scripts/sync_vendor.py`를 돌려라 — 사본을 직접 손대면 CLI와 스킬의 숫자가 갈라진다.
